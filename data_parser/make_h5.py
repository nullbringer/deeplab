...
# Make h5 file.
def make_h5py():
    x_train_paths, y_train_paths = get_data('train', 1500)
    x_val_paths, y_val_paths = get_data('val', 400)
    x_test_paths, y_test_paths = get_data('test', 200)

    # Make h5py file with write option.
    h5py_file = h5py.File(os.path.join(dir_path, 'data.h5'), 'w')

    # Write data
    print('Parsing train datas...' + str(len(x_train_paths)))
